...
# ⚠️ Giữ NGUYÊN VĂN như trong 02_prompt_hub_ab_routing.py để kết quả so sánh được.
SYSTEM_V1 = (
...
def run_rag(retriever, llm, prompt, question: str) -> dict:
    ...
    # RAGAS cần từng đoạn riêng biệt → giữ nguyên dạng list[str]
    contexts = [doc.page_content for doc in docs]

    # Chỉ ghép thành chuỗi khi truyền vào biến {context} của prompt
    ctx_str = "\n\n".join(contexts)
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    # Giảm max_workers và tăng timeout/retries: mạng sandbox không chịu được
    # 16 request đồng thời mặc định của RAGAS, gây TimeoutError hàng loạt.
    run_config = RunConfig(timeout=180, max_retries=15, max_wait=90, max_workers=4)
...
def main():
    ...
    # Sao chép luôn sang evidence/ để nộp bài

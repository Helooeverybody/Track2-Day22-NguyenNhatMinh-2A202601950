...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value; nếu phát hiện, coi là FAIL và trả kèm fix_value đã redact.
        on_fail=OnFailAction.FIX sẽ khiến Guardrails thay output bằng fix_value.
        ...
        # Redact tích lũy trên redacted_text để nhiều loại PII trong cùng
        # một câu đều được xử lý.
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        JSON hợp lệ ngay từ đầu → PassResult() (không cần sửa).
        JSON lỗi nhưng sửa được → FailResult(fix_value=<JSON đã sửa, format đẹp>)
        so on_fail=OnFailAction.FIX thay output bằng bản đã sửa.
        JSON không thể sửa → FailResult(fix_value=<JSON lỗi fallback>).
        """
        # 1) Thử parse trực tiếp — đã hợp lệ, không cần sửa
        try:
            json.loads(value)
            return PassResult()
        ...
        try:
            ...
            return FailResult(
                error_message="JSON không đúng định dạng, đã tự động sửa",
                fix_value=json.dumps(parsed, indent=2),
            )
        except json.JSONDecodeError as e:
            # 3) Không thể sửa → trả JSON lỗi chuẩn làm fix_value
            fallback = json.dumps(
                {"error": "Không thể phân tích JSON", "raw": value[:200]},
                indent=2, ensure_ascii=False,
            )
            print(f"  ❌ Không sửa được JSON → dùng fallback")
            return FailResult(
...
def demo_pii_guard():
    ...
    # ⚠️ on_fail truyền vào CONSTRUCTOR của validator, không phải Guard.use()
